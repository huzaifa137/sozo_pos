    ...
    <div class="settings-grid">
        ...
        <div class="settings-card">
            ...
            <form action="{{ route('categories.store') }}" method="POST" id="addCatForm">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="code" id="codeField" value="{{ old('code') }}" placeholder="e.g. electronics"
                        required>
                ...
                <div class="toggle-row">
                    <div>
                        ...
                        <div class="toggle-sub">Inactive categories won't appear in dropdowns</div>
                    </div>
                ...
                <div class="form-actions">
                    <button type="submit" class="btn-submit" id="addCatBtn">
        ...
        <div class="table-card">
            <div class="table-card-hdr">
                ...
                <span class="table-count">{{ $categories->count() }} total</span>
            </div>
                ...
                <table class="data-table">
                    ...
                    <tbody>
                        @foreach($categories as $cat)
                            <tr>
                                <td>
                                    <div style="font-weight:600">{{ $cat->display_name }}</div>
                                    <div style="font-size:.75rem;color:var(--muted)">{{ $cat->name }}</div>
                                </td>
                                <td><span class="code-chip">{{ $cat->code }}</span></td>
                                <td>
                                    <span style="font-family:var(--font-head);font-weight:700;font-size:.9rem;color:var(--accent)">
                                        {{ $cat->inventory_items_count }}
                                    </span>
                                </td>
                                <td>
                                    <span style="display:inline-flex;align-items:center;font-size:.78rem">
                                        <span class="status-dot {{ $cat->is_active ? 'status-active' : 'status-inactive' }}"></span>
                                        {{ $cat->is_active ? 'Active' : 'Inactive' }}
                                    </span>
                                </td>
                                <td style="color:var(--muted);font-size:.78rem">{{ $cat->created_at->format('d M Y') }}</td>
                                <td>
                                    <div style="display:flex;gap:.4rem">
                                        <button class="action-btn ab-edit" onclick="openEditModal(
                                                    {{ $cat->id }},
                                                    '{{ addslashes($cat->name) }}',
                                                    '{{ addslashes($cat->display_name) }}',
                                                    '{{ $cat->code }}',
                                        ...
                                        <form class="del-form" action="{{ route('categories.destroy', $cat) }}" method="POST"
                                            style="display:inline">
                                            @csrf @method('DELETE')
                                            <button type="submit" class="action-btn ab-del" data-name="{{ $cat->display_name }}"
                                                data-count="{{ $cat->inventory_items_count }}">
                                                <svg width="12" height="12" fill="none" stroke="currentColor" stroke-width="2"
                                                    viewBox="0 0 24 24">
        ...
        <div class="modal-box">
            ...
            <form id="editCatForm" method="POST">
                ...
                <div class="form-actions">
                    ...
                    <button type="submit" class="btn-submit" id="editCatBtn">
                        <svg width="15" height="15" fill="none" stroke="currentColor" stroke-width="2.5"
                            viewBox="0 0 24 24">
                            <path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z" />
                        </svg>
...
@push('scripts')
    <script>
        ...
        document.getElementById('addCatForm').addEventListener('submit', function (e) {
            ...
            Swal.fire({
                title: 'Save Category?',
                text: 'Add this category to your system?',
                icon: 'question',
                background: 'var(--surface)',
                color: 'var(--text)',
        ...
        /* ── EDIT MODAL ── */
        function openEditModal(id, name, displayName, code, description, isActive) {
